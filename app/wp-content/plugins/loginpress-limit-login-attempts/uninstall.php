	...
	if ( function_exists( 'is_multisite' ) && is_multisite() ) {
		...
		foreach ( $blog_ids as $blog_id ) {
			...
			if ( 'on' == $is_delete_table ) {
				drop_loginpress_limit_login_attempts_details_table(); // delete table for blog id
			}
		}
		...
		if ( 'on' == $is_delete_table ) {
			drop_loginpress_limit_login_attempts_details_table(); // normal deactivaton delete table
		}
	}
