...
//var_dump($where);
	...
	// specific referrals
		...
		$cache_key = (true === $count) ? md5('cmc_coins_list_count' . serialize($args)) : md5('cmc_coins_list_' . serialize($args));
	...
	// specific referrals
		...
		$cache_key = (true === $count) ? md5('cmc_top_c_coins_count' . serialize($args)) : md5('cmc_top_c_coins_' . serialize($args));
...
function wp_insert_rows($row_arrays = array(), $wp_table_name, $update = false, $primary_key = null) {
	...
	$query         = "";
	$query_columns = "";
	$floatCols=array('price','percent_change_24h','market_cap','total_volume','circulating_supply','coin_status');
	...
	public function create_table()
	{
		...
		coin_status varchar(250) NOT NULL DEFAULT 'enable',
		PRIMARY KEY (id)
