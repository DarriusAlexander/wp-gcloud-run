...
namespace CryptocurrencyExchangesListProREG;
	...
	class CryptocurrencyExchangesListProBase {
    	public $key = "ECF94F293AE332B2";
    	private $product_id = "5";
    	private $product_base = "celp";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_celp_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=celp_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
					}
