					...
					foreach ( $results as $result ) {
						...
						foreach ( $coupon_data as $key => $value ) {
							...
							if ( $key == 'product_ids' || $key == 'exclude_product_ids' ) {
								...
								$data[ $id ][ $key ] = ( isset( $coupon_data[ $key ] ) ) ? implode( '|', maybe_unserialize( stripslashes( $coupon_data[ $key ] ) ) ) : '';
							} elseif ( $key != 'ID' ) {
								$data[ $id ][ $key ] = (is_serialized( $value )) ? implode( ',', maybe_unserialize( stripslashes( $value ) ) ) : $value;
							}
						}
					}
				}
