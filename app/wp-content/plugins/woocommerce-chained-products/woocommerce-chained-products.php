...
if ( ! class_exists( 'WC_Chained_Products' ) ) {
    ...
    class WC_Chained_Products {
        ...
        public function __construct() {
		    ...
		    add_filter( 'woocommerce_product_is_on_sale', array( $this, 'sa_set_chained_product_to_sale' ), 15, 2 );
        }

		/**
		 * Function to set all the chained products in the cart to on-sale
		 *
		 * @param string $on_sale
		 * @param WC_Product $product
		 * @return string $on_sale
		 */
        public function sa_set_chained_product_to_sale( $on_sale, $product ) {
        	...
        	if ( ! empty( $cart ) ) {
	        	...
	        	foreach ( $cart_contents as $cart_item_key => $cart_item ) {
	        		$in_cart_product_id = ( isset( $cart_item['variation_id'] ) && ! empty( $cart_item['variation_id'] ) ) ? $cart_item['variation_id'] : $cart_item['product_id'];

	        		if ( isset( $cart_item['chained_item_of'] ) && ( $product_id === $in_cart_product_id ) ) {
	   					$on_sale = true;
	        		}
	        	}
	        }
