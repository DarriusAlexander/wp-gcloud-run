...
if ( ! class_exists( 'woocommerce_msrp_frontend' ) ) {
	...
	class woocommerce_msrp_frontend {
		...
		function __construct() {
			...
			add_filter( 'woocommerce_rest_prepare_product_object', array( $this, 'rest_api_price_output_v2_simple' ), 10, 3 );
			add_filter( 'woocommerce_rest_prepare_product_variation_object', array( $this, 'rest_api_price_output_v2_variation' ), 10, 3 );
			add_filter( 'woocommerce_rest_pre_insert_product_object', array( $this, 'rest_api_maybe_update_msrp_v2' ), 10, 3 );
			add_filter( 'woocommerce_rest_pre_insert_product_variation_object', array( $this, 'rest_api_maybe_update_msrp_v2' ), 10, 3 );
		...
		function get_msrp_for_variation( $current_product ) {
			if ( is_callable( array( $current_product, 'get_id' ) ) ) {
				// WC > 2.7.0
				return get_post_meta( $current_product->get_id(), '_msrp', true );
			} else {
				// WC < 2.7.0
				return get_post_meta( $current_product->id, '_msrp', true );
			}
		}
		...
		function get_savings_for_variable_product( $current_product, $type = 'amount' ) {
			...
			foreach ( $children as $child ) {
				...
				$msrp = get_post_meta( $child, '_msrp', true );
		...
		public function rest_api_maybe_update_msrp_v2( $product, $request, $creating ) {
			...
			update_post_meta( $product->get_id(), $key, $request['msrp_price'] );
			return $product;
		}
