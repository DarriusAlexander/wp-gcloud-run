...
onclick:function(){var b=jQuery(this.$el);b.hasClass("themify_fa_toggle")||b.addClass("themify_fa_toggle");if(document.getElementById("themify_builder_site_canvas_iframe")&&!a)var a=document.getElementById("themify_builder_site_canvas_iframe").contentWindow.Themify_Icons;void 0!==a&&(a.target=b.prev(),a.showLightbox(a.target.val()))}}]});g.push(b)});d.windowManager.open({title:a.label,body:g,onSubmit:function(c){c=this.toJSON();c.selectedContent=d.selection.getContent();var b=wp.template("themify-shortcode-"+
a.id);d.insertContent(b(c))}})}}}}var e=[];jQuery.each(themifyEditor.shortcodes,function(a,c){c.id=a;if("object"==typeof c.menu){var d=[];jQuery.each(c.menu,function(a,c){c.id=a;d.push(f(c))});e.push({text:c.label,menu:d})}else e.push(f(c))});d.addButton("btnthemifyMenu",{type:"menubutton",text:"",image:themifyEditor.editor.icon,tooltip:themifyEditor.editor.menuTooltip,menu:e})});
