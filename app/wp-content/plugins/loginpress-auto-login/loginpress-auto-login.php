...
if ( ! class_exists( 'LoginPress_AutoLogin' ) ) :
      ...
      <th class="loginpress_action">' . esc_html__( 'Action', 'loginpress-auto-login' ) . '</th>
			...
			if ( ! empty( $autologin_user ) ) {
				...
				foreach ( $autologin_user as $user ) {
					...
					$loginpress_user_autologin_nonce = wp_create_nonce( 'loginpress-user-autologin-nonce' );
					$html .= '<tr id="loginpress_user_id_' . $user->ID . '" data-autologin="' . $user->ID . '"><td>' . $user_info->ID . '</td><td class="loginpress_user_name">' . $user_info->user_login . '</td><td >' . $user_info->user_email . '<input type="hidden" class="loginpress__user-autologin_nonce" name="loginpress__user-autologin_nonce" value="' . $loginpress_user_autologin_nonce . '"></td><td class="loginpress_autologin_code"><span class="autologin-sniper"><img src="' . admin_url( 'images/wpspin_light.gif' ) . '" /></span><p>' . site_url() . '/?loginpress_code=' . get_user_meta( $user->ID, 'loginpress_autologin_code', true ) . '</p></td><td><input type="button" class="button loginpress-new-link" value="' . esc_html__( 'New', 'loginpress-auto-login' ) . '" id="loginpress_create_new_link" /> <input type="button" class="button loginpress-del-link" value="' . esc_html__( 'Delete', 'loginpress-auto-login' ) . '" id="loginpress_delete_link" /></td></tr>';
				}
			} else {
    ...
    // }
        ...
        wp_die( 'No cheating, huh!' );
			...
			echo site_url() . '/?loginpress_code=' . get_user_meta( $user_id, 'loginpress_autologin_code', true );
			wp_die();
		}
